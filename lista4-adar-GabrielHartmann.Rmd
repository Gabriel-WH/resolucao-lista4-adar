...
exerc_1 <- soi %>%
  pivot_longer(
    ...
    names_to = "mes",
    values_to = "soi",
    values_transform = list(mes = as.numeric) # desnecessario se tem o mutate com as.integer abaixo, tanto que se comentares esta linha nao fará efeito no resultado.
  ) %>%
  rename_with( # muito bom!
    .data = .,
    .fn = tolower
  ) %>%
  mutate(
    .data = .,
    across(mes, as.integer) # across eh util quando aplicado a mais de uma variavel
  )
...
```{r}
exerc_3 <- poluentes %>%
  select(., estacao, poluente, duracao)
...
```{r}
filter(exerc_5b, is.na(tmin) | is.na(tmax)) # correto, mas com tantos códigos fica difícil saber qual sua resposta.
...
Como podemos observar, são 5 datas que possuem dados faltantes. Mas, analisando a saída do recorte das observações antes e depois das datas com dados faltantes, vemos que temos dados faltantes entre as datas de 28 de fevereiro e 1º de março (3 dias de dados faltantes para ser mais específico), bem como entre 30 de abril e 05 de maio (1 dia). O que fica claro é que esse conjunto de dados foi gerado a partir de um comando, uma regra, que considerava que todos o meses têm 31 dias.
...
> JDT: note que a estrutura dos dados não é idêntica ao resultado fornecido. Sua resposta tem date no formato `POSIXlt`. Os dados de referência no formato `dttm`. No curso não foi indicado  o uso da `strptime()` mas da `as.POSICXct()`, como [aqui](https://lhmet.github.io/adar-ebook/outros-tipos-de-dados-derivados.html#datas-e-hor%C3%A1rios).
...
```{r jdt-dica11, echo = FALSE, comment = "JDT>"}
exerc_10 %>% 
...
exerc_12 <- dados_rs_08_16 %>%
  ...
  summarise(
    ...
    disp_med = 100 * sum(!is.na(tair)) / n() # DO not repeat yourself
  ) %>%
  rename(
    .data = .,
